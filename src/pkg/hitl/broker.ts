...
export async function listQuestions(input: BrokerOptions = {}): Promise<HitlQuestion[]> {
  ...
  for (const file of files) {
    ...
    const key =
      typeof parsed.key === "string" && parsed.key.trim().length > 0 ? parsed.key.trim() : safeKey;
...
export async function getQuestionByKey(
  ...
  const safeKey = toSafeKey(normalized);
  const questions = await listQuestions(input);
  return (
    questions.find((question) => question.safeKey === safeKey || question.key === normalized) ??
    null
  );
}

export async function submitQuestion(
  payload: { key: string; question: string; timestamp?: number; pid?: number },
  input: BrokerOptions = {}
): Promise<{ key: string; safeKey: string; path: string }> {
...
export async function cancelQuestion(
  key: string,
  input: BrokerOptions = {}
): Promise<{ key: string; safeKey: string; removed: boolean }> {
  const safeKey = toSafeKey(key);
  const dir = await ensureHitlDir(input);
  const questionPath = path.join(dir, `${safeKey}.question`);
  const existed = await stat(questionPath)
    .then(() => true)
    .catch(() => false);
...
export async function listBrokerState(
  input: BrokerOptions = {}
): Promise<Map<string, BrokerStateEntry>> {
  const dir = await ensureHitlDir(input);
  const files = await readdir(dir).catch(() => []);
  ...
  for (const file of files) {
    ...
    const questionPath = path.join(dir, `${safeKey}.question`);
    const answerPath = path.join(dir, `${safeKey}.answer`);
    const questionMtimeMs = await stat(questionPath)
      .then((row) => row.mtimeMs)
      .catch(() => 0);
    const answerStat = await stat(answerPath).catch(() => null);
